...
void main(void) {
	...
	vec4 p13 = 0*neighbor(0,-3);
	vec4 p14 = 0*neighbor(0,3);
	vec4 p15 = 0*neighbor(3,0);
	vec4 p16 = 0*neighbor(-3,0);
	...
	float a = 1.0 - pow(distance(b,vec2(0.0))/330, 0.5);
	...
	if(sum.r == 3.0)
		...
		gl_FragColor.rgb = c.rgb - vec3(0.71) - a;
	else
		gl_FragColor.rgb = c.rgb + vec3(0.95);
